    ...
    if st.button("Run"):
        if features:
            if usekfold:
                ...
                if btn_log:
                    fig_log = plt.figure()
                    plt.bar(n - 0.2, log_train_list, color='r', width=0.4, label="Log Loss on Training Set")
                    plt.bar(n + 0.2, log_test_list, color='g', width=0.4, label="Log Loss on Test Set")
                    plt.ylabel('Log Loss')
                    plt.xlabel('Folds')
                    ...
                    plt.bar('Average Log Loss on Training Set', log_avg_train, color='r')
                    plt.bar('Average Log Loss on Test Set', log_avg_test, color='g')   
                    plt.ylabel('Log Loss')
                    plt.xlabel('Training and Test Datasets')
                ...
                if btn_f1:
                    ...
                    st.pyplot(fig_f1)
                if btn_log:
                    st.write('Log Loss on Training Set: ', log_train)
                    st.write('Log Loss on Test Set: ', log_test)
                    fig_log = plt.figure() 
                    plt.bar('Log Loss on Training Set', log_train, color='r')
                    plt.bar('Log Loss on Test Set', log_test, color='g')   
                    plt.ylabel('Log Loss')
                    plt.xlabel('Training and Test Datasets')
                    plt.title('Log Loss')
                    st.pyplot(fig_log)
